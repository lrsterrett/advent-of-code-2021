...
ebc abcgfde be aedbcf egadfc gbdfc fcdae baef bdceag efcbd | aegdfc efab fedbc cbedafg`
    .split("\n")
    .map((row) => {
      return row.split(" | ").map((string) => string.split(" "));
    });

let sumOfOutputs = 0;

for (const display of input) {
  sumOfOutputs += getOutputOfDisplay(display);
}

// 416634 is too low
console.log(sumOfOutputs);

function getOutputOfDisplay(display) {
  const knownDigits = getKnownDigits(display[0]);
  let outputString = "";

  for (const digit of display[1]) {
    const sortedDigit = digit.split("").sort().join("");
    outputString += knownDigits.indexOf(sortedDigit);
  }

  return parseInt(outputString);
}

function getKnownDigits(sample) {
  const knownDigits = new Array(10);
  const zeroOrSixOrNine = [];
  const twoOrThreeOrFive = [];
  ...
  for (const digit of sample) {
    ...
    if (digit.length == 2) {
      knownDigits[1] = sortedDigit;
    } else if (digit.length == 3) {
      knownDigits[7] = sortedDigit;
    } else if (digit.length == 4) {
      knownDigits[4] = sortedDigit;
    } else if (digit.length == 7) {
      knownDigits[8] = sortedDigit;
    } else if (digit.length == 5) {
      twoOrThreeOrFive.push(sortedDigit);
    } else {
      zeroOrSixOrNine.push(sortedDigit);
    }
  }
  ...
  // Find 6
  for (let i = 0; i < zeroOrSixOrNine.length; i++) {
    const digit = zeroOrSixOrNine[i];
    const one = knownDigits[1];
    const regexOne = new RegExp(one[0]);
    const regexTwo = new RegExp(one[1]);
    if (!regexOne.test(digit)) {
      knownDigits[6] = digit;
      topRightLetter = one[0];
      zeroOrSixOrNine.splice(i, 1);
      break;
    } else if (!regexTwo.test(digit)) {
      knownDigits[6] = digit;
      topRightLetter = one[1];
      zeroOrSixOrNine.splice(i, 1);
      break;
    }
  }
  ...
  // Find 5
  for (let i = 0; i < twoOrThreeOrFive.length; i++) {
    const digit = twoOrThreeOrFive[i];
    const regex = new RegExp(topRightLetter);

    if (!regex.test(digit)) {
      knownDigits[5] = digit;
      twoOrThreeOrFive.splice(i, 1);
      break;
    }
  }

  let bottomLeftLetter;

  // Find bottom left letter
  for (const letter of knownDigits[6]) {
    const regex = new RegExp(letter);

    if (!regex.test(knownDigits[5])) {
      bottomLeftLetter = letter;
    }
  }

  // Find 2 and 3
  for (const digit of twoOrThreeOrFive) {
    const regex = new RegExp(bottomLeftLetter);
  ...
  // Find 0 and 9
  for (const digit of zeroOrSixOrNine) {
    const regex = new RegExp(bottomLeftLetter);
