...
const pathCount = search("start", { start: true }, 0);
...
function search(currentCave, visited, pathCount) {
  const nextCaves = getNextCaves(currentCave, visited);

  for (const cave of nextCaves) {
    if (cave == "end") {
      pathCount++;
    } else {
      pathCount = search(cave, { ...visited, [cave]: true }, pathCount);
    }
  }
